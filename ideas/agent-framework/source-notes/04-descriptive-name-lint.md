...
## Idea
1. Add a warning-only name check to the repo-root `check-framework.sh` (or a small backing
   script in `scripts/` if the shell logic becomes awkward).
...
## Likely home
Repo-root `check-framework.sh` warning section + optional small backing script in `scripts/`.
No new persona needed.
