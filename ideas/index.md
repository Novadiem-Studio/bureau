...
| **Stakeholder Companion** — project-agent status reader + LLM log summariser + developer RecentActivity panel over bureau run artifacts | `20260614-stakeholder-companion` (complete, merged to main) | `done/stakeholder-companion.md` |
...
| **Rheo Memory Track 3 — Curation & Quality** — `entity_confirm`/`entity_supersede` MCP tools, extraction dedup signal (`probable_duplicate_of` + levenshtein), `graph.jsonl` backup, MCP arg-shape guard (edges + compaction cut by design at the design-model checkpoint) | `20260622-rheo-memory-track3` (built + merged to mot `main`, local; not pushed) | `done/rheo-memory-track3-curation.md` |
...
| **Ministry System blueprint** — government ops dashboard: M.O.T., M.O.I., domain ministries | `20260614-ministry-system-blueprint` + `20260617-moi-spine-reconcile` + `20260617-moi-spine-build` | **Phase 1 (M.O.T.) BUILT + LIVE** at rheo.ca/mot; **Phase 2 (M.O.I. spine) BUILT + LIVE** — hourly intake routine, audit/prompt_hash spine, System Health panel, reopen-noise fix shipped; **P2.5 (briefing) BUILT + LIVE** — `notify_robin` MCP tool, `/briefing` Telegram command, daily 7am briefing running (`20260622-rheo-briefing`); Phases 3+ not started | `in-progress/ministry-system.md` |
...
| **Rheo Memory Track 4 — Synthesis & Proactive Recall** | [`not-started/rheo-memory-track4-synthesis.md`](not-started/rheo-memory-track4-synthesis.md) | Single `memory_context` boot call, `topic_thread_summarize` cross-session synthesis, scheduled nightly maintenance, historical session backfill, MOT web UI for entity/topic/procedural browsing. Depends on Track 3 (now built + merged). |
| **M.O.T. private-ticket leak (key-only)** | [`not-started/mot-private-ticket-leak-key-only.md`](not-started/mot-private-ticket-leak-key-only.md) | HIGH / priority — pre-existing privacy bug on mot `main`: a private ticket is visible to API-key-only callers (private-routes `AC-PRIVATE` red identically at base `e9d7311`). Surfaced by the Track 3 run; fix the route gating. `bug-fix` workflow. |
| **M.O.T. ESLint config + cleanup** | [`not-started/mot-eslint-config-and-cleanup.md`](not-started/mot-eslint-config-and-cleanup.md) | LOW — repo ships no committed ESLint config so `npm run lint` is interactive; plus one unused import in `components/FilterChips.tsx`. Commit a Next.js eslint config + drop the import. Surfaced by the Track 3 run. |
| **M.O.L. `claude_cli` provider** | [`not-started/mol-claude-cli-provider.md`](not-started/mol-claude-cli-provider.md) | Route mol's LLM calls through Robin's Claude subscription headless via `claude -p` (like rheo-bot) instead of per-token APIs; proposal drafts on Opus 4.8. Fixes weak haiku copy. Global switch (`LLM_PROVIDER=claude_cli`), Opus for proposal/humanize + Haiku for scoring/parse, sanitized env to force subscription auth, loud-on-failure. Removes the rejected blind em-dash strip. |
