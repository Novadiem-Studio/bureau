...
> **Recommended tier:** strong — QA runs on a capable tier, and escalates on demonstrated need. **Independence comes from FRESH CONTEXT, not from differing from the author's model**: a reviewer that has never seen the design argued gives real objections whether or not it shares a model with the coder. Do not route a review by model identity; route it by capability and by what a bounce has shown. (on Claude an opus author gets a sonnet reviewer). Independent fresh-context review required. Frontier/escalated only on an explicit human ask; `final_gate` means a fresh cold pass, not a costlier model (Robin, 2026-09-16; `docs/model-routing-and-cast.md` § Escalation ladder).
