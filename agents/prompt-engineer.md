...
Mode appendix:  if spawned for execute-plan or design-build prompt-folder work, read `agents/modes/spellwright-execute-plan.md`.
...
## Execute-plan prompt folders

For `execute-plan` and `design-build`, read `agents/modes/spellwright-execute-plan.md` before
writing the prompt folder. That appendix owns coder tags, reviewability lines, and build-party
domain gotchas.
