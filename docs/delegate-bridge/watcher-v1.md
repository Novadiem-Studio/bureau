...
computes and stages itself, for the reviewer to copy into `Artifact-hash`), every supplementary
artifact in the packet with its digest, `$CTX/artifacts.sha256` (the manifest the reviewer copies
into `Artifacts-read`), `$CTX/log-slice.md`,
