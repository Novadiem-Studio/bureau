...
3. Create this run's **run dir** — `output/runs/<yyyymmdd>-<task-slug>/` — and initialize
   `state.json` + `log.md` inside it. Every output file for this run lives there (see "Run
   directory" in `agents/orchestrator.md`); this is what lets multiple sessions run
   concurrently on one install. (Legacy: an in-flight run with a top-level
   `output/state.json` finishes in place.)
