...
| [build-review-cold](build-review-cold.md) | A self-contained change in existing code you can just build — no fresh spec, no plan to decompose — where you want the speed-first third gear: build solo, then ONE cold diff review only if the diff can fail silently. NOT for a new feature (→ feature), a canon/framework change (→ execute-plan), or a multi-surface / high-blast-radius diff (→ escalate) | mixed | The Conductor dispatches the domain's coder (Mage/Systemsmith/Mechanic) to build solo → **self-gate**: does the diff write durable state, move value, gate access, fire something irreversible, or race on shared data where a wrong result won't announce itself? → No: close out at dev · Yes: ONE cold build-diff Challenger (refute-first, diff-only, "what corrupts/leaks/miscounts/races silently?") → The Conductor/Robin adjudicates → dev-verified. Stops at dev; external-action gate still applies |
