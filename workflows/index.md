...
| [run-series](run-series.md) | Robin points at a run-series plan (`INDEX.md` + run cards; charter in that dir or its parent) and wants it driven run to run — launched once, left to run as long as the token budget allows, Robin pinged only at real gates. Also "run the series" / Envoy. NOT a single run (just run it) | execute | The Envoy loop: resolve the plan, hold one run's watch, verify close-outs, cold second-pass at each PR gate, relay and launch the next unblocked run via `claude --bg` (per supervisor-spawned-runs.md), keep INDEX/HANDOFF/STATE small, stop at 80% session/weekly usage; fills LORE XVIII, The Envoy |
...
| Point the Envoy at this build-plan and go run to run | `run-series` |
