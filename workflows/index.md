...
| [run-series](run-series.md) | You have a mapped-out SERIES of Bureau runs (a run-series INDEX + a campaign charter of decide-vs-escalate authority) and want it driven to completion autonomously — launched once, left to run as long as the token budget allows, Robin pinged only at real gates. NOT a single run (just run it) | execute | The Envoy loop: self-scheduled ticks monitor the active run's RUN_DIR, verify close-outs, spawn a cold second-pass (Challenger/escalated) at each PR gate, relay verdicts and launch each next unblocked run via `claude --bg` (per supervisor-spawned-runs.md), keep INDEX/HANDOFF, stop at 80% session/weekly usage; fills LORE XVIII, The Envoy |
...
| [write-topic-overview](write-topic-overview.md) | Robin wants the written guide above a topic hub's article list (devweb.org `/topics/<pillar>`, `content/topics/<pillar>.mdx`): 1–3 guide-shaped pages that answer the pillar's query and map its essays + glossary terms into a reading order. NOT a definition (→ write-glossary) or an essay (→ write-article) | mixed | Thin sibling of write-glossary: same eleven steps with a guide page contract (400–700 words, ≥3 essay links, ≥2 term links, boundary + where-to-start), whole-pillar grounding table, topic Grok pass, proofreader also checks every link resolves and every essay claim matches the essay → publish CLEAR pages to content/topics/ |
| [write-data-page](write-data-page.md) | Robin wants a first-party data page (devweb.org `/data/<slug>`): figures derived by script from the studio's own run records (tokens by role, API-equivalent cost with the arithmetic shown, dated price basis), the dataset file, and optional RunTables on the essays those runs produced. NOT an essay about costs (→ write-article) | mixed | Thin sibling of write-glossary: mechanical grounding (`scripts/run-cost-dataset.py` → tables.md/coverage.md), Scribe writes prose around verbatim tables, `check-numbers.py` gates Format, reconcile and proofread (no number may differ from the tables), data Grok pass, publish page + `public/data/<slug>.json` + `run:` frontmatter on named essays |
| [write-evidence-page](write-evidence-page.md) | Robin wants an evidence page (devweb.org `/evidence/<slug>`): a "does X work" question answered from the run records — what was measured, the runs it stands on, the answer, its limits — with a grade on the EVIDENCE (strong/mixed/limited/insufficient), never a recommendation. NOT an opinion essay (→ write-article) | mixed | Thin sibling of write-data-page: mechanical grounding (`scripts/review-evidence-dataset.py` → anonymized corpus tables: critic loops, rework ratio, cold-review decisions, proofreader holds, Challenger findings taxonomy), Scribe writes the five fixed sections around verbatim tables and proposes the grade, `check-numbers.py` gates, proofreader also checks the grade matches the stated limits and that nothing recommends a practice, publish CLEAR pages + `public/data/review-evidence.json` |
...
| Write the overview for the memory hub on devweb (the guide above /topics/memory) | `write-topic-overview` |
| Publish what a Bureau article run costs, by role, from the accounting files | `write-data-page` |
| Does an AI code reviewer catch real problems? Answer it from our runs, graded | `write-evidence-page` |
