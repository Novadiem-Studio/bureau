...
command: |
  ...
  [ "$n" = "2" ] || { echo "FAIL: expected 2 fable-override defects, got $n"; printf '%s\n' "$out"; rm -rf "$TMPF"; exit 1; }
  ...
  # Host-neutral: with a resolved routing whose escalated tier is gpt-6-astra, an
  # Astra spawn is checked and a fable spawn is not.
  cat > "$RD/model-routing.json" <<'JSON'
  {"runtime":"openai","tiers":{"strong":{"model":"gpt-5.6-sol"},"frontier":{"model":"gpt-5.6-sol"},"escalated":{"model":"gpt-6-astra"}}}
  JSON
  ...
  # Negative control: give the two defects a hand-written reason; the check goes quiet.
  write_log
