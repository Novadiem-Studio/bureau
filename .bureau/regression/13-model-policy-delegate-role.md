...
command: |
  ...
  jq -e '.roles.delegate | (.default_tier=="strong") and (.allowed==["standard","strong","frontier","escalated"]) and (.escalate_when|length==9) and (.deescalate_when==["author_ran_strong"]) and (.cold_reviewer.rule=="differs_from_author") and (.cold_reviewer.author_strong=="standard") and (.cold_reviewer.author_standard=="strong")' $ROOT/config/model-policy.v2.json
