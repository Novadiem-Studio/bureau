name: model-pass.sh exits 3 when output is over 300% of the input byte count
...
command: |
  ...
  # Mock curl: 200, finish_reason stop, but content is a runaway ~600 chars (>300% of ~71-byte draft).
  mkdir -p "$WORK/bin"
  cat > "$WORK/bin/curl" <<'MOCK'
  #!/usr/bin/env bash
  big="$(printf 'runaway %.0s' $(seq 1 80))"
  body="$(jq -n --arg c "$big" '{choices:[{finish_reason:"stop",message:{content:$c}}]}')"
  printf '%s\n%s' "$body" '200'
  MOCK
  chmod +x "$WORK/bin/curl"
  # ~71-byte draft; 300% ceiling is ~213 bytes; the ~640-byte runaway is well over.
  ...
  test ! -f "$OUT" || { echo "FAIL: out-file was written despite over-length output" >&2; exit 1; }
  echo "PASS: length-delta over 300% -> exit 3, out-file untouched"
