name: model-policy challenger role — standard allowed, NO author_ran_strong deescalate, no final_gate / high_stakes escalation triggers
command: |
...
expected: exit 0 — jq prints true; nonzero/false if the Challenger role drifts: default tier, the four-tier allowed set with standard (Robin, 2026-09-16), the two remaining escalation triggers, the ABSENCE of author_ran_strong deescalate slug, or the return of final_gate / high_stakes_backend_or_security as escalation triggers. Mutation: re-add "final_gate" to escalate_when → false.
