name: account-tokens G1 golden — all seven derived metrics exact (AC 13/14/15)
command: |
  ...
  printf '%s\n' \
    ...
    'SPAWN-TOKEN-EVENT: {"attempt_id":"architect-2","agent_id":"agent-ccc","at":"2026-07-05T00:02:00Z","turns":2,"tokens":{"input":60,"cache_creation":90,"cache_read":150,"processed":300,"output":15}}' \
  ...
  rm -rf "$TMPF"
  echo "PASS"
  # Mutation: drop the agent_id/session_id dedup or swap rework:true->the attempt
  # counter and processed_total/rework_ratio shift off 2000/0.15 -> fixture fails.
expected: exit 0; stdout "PASS"; processed_total=2000(exact) rework_ratio=0.15 tokens_per_loop=1000 active_spawn_time_s=120(exact) minutes_per_loop=1.0 human_wait_total_s=300 active_vs_blocked_ratio=0.4 conductor exact output estimated
phase: 04 · feature
