name: advisory-lock-call
command: |
  ROOT="${ROOT:-$(git rev-parse --show-toplevel)}"
  TMP="$(mktemp -d)"
  trap 'rm -rf "$TMP"' EXIT
  # spec.md contains a fenced block with the flock advisory-lock pattern (exact literal from spec FR 5d)
  cat > "$TMP/spec.md" <<'SPEC'
  **FR 1 — Only FR**
  A requirement.
  
  ```
  flock /tmp/lockfile command_to_run
  ```
  SPEC
  ...
  echo "$OUT" | grep -qF 'advisory-lock-call' || { echo "FAIL: output does not name advisory-lock-call"; exit 1; }
...
expected: |
  ...
  script output includes advisory-lock-call and names spec.md with approx line number
